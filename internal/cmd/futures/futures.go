...
func NewCmdFutures(f *factory.Factory) *cobra.Command {
	c := &cobra.Command{
		...
		Long: "Operate on the futures product surface.\n\n" +
			...
			"the `market` subtree is public and can be used without a secret when an endpoint exists.\n\n" +
		...
		Example: "# Show the latest market state for BTCUSDT\n" +
			...
			"# List active triggers for BTCUSDT\n" +
	...
	c.AddCommand(orderCmd, triggerCmd, positionCmd, balanceCmd, marketCmd)
	return c
}

...
func runShow(ctx context.Context, opts *ShowOptions) error {
	...
	return f.IO.Render(resp, func() error {
		return f.IO.Object([]output.KV{
			...
			{Key: "Leverage", Value: emptyDash(resp.Leverage) + "x"},
			{Key: "Price", Value: resp.Price},
			{Key: "Avg Price", Value: emptyDash(resp.AvgPrice)},
			{Key: "Size", Value: resp.Volume},
			{Key: "Filled", Value: resp.Filled},
			{Key: "Left", Value: emptyDash(resp.Left)},
			{Key: "Filled Value", Value: emptyDash(resp.DealStock)},
			{Key: "Fee", Value: emptyDash(resp.DealFee)},
